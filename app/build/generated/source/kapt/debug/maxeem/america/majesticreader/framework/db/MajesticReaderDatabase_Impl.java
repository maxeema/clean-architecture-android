    ...
    final SupportSQLiteOpenHelper.Callback _openCallback = new RoomOpenHelper(configuration, new RoomOpenHelper.Delegate(1) {
      ...
      protected void validateMigration(SupportSQLiteDatabase _db) {
        ...
        if (! _infoBookmark.equals(_existingBookmark)) {
          throw new IllegalStateException("Migration didn't properly handle bookmark(maxeem.america.majesticreader.framework.db.BookmarkEntity).\n"
                  + " Expected:\n" + _infoBookmark + "\n"
                  + " Found:\n" + _existingBookmark);
        }
        ...
        if (! _infoDocument.equals(_existingDocument)) {
          throw new IllegalStateException("Migration didn't properly handle document(maxeem.america.majesticreader.framework.db.DocumentEntity).\n"
                  + " Expected:\n" + _infoDocument + "\n"
                  + " Found:\n" + _existingDocument);
        }
      }
